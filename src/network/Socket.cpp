...
{
  std::memcpy(&this->sharedSocket->peerAddress, address->ai_addr, address->ai_addrlen);
}

bool Socket::operator==(const Socket &other) const
{
  return this->sharedSocket->socketFileDescriptor == other.sharedSocket->socketFileDescriptor;
}
...
{
  return ::recv(this->sharedSocket->socketFileDescriptor, buffer, capacity, MSG_DONTWAIT);
}
...
bool Socket::send()
{
  ...
  {
    const ssize_t result =
        ::send(this->sharedSocket->socketFileDescriptor, buffer + sent, length - sent, 0);
